...
public class QuizHistoryData {
    ...
    public QuizHistory storeQuizHistory(QuizHistory quiz ) {
        ...
        latestID = id; // save the most recently used quiz ID
    ...
    public void processAnswer(int questionNum, boolean isCorrect) {
        QuizHistory quizHistory = getLatestQuizHistory();
        int numAnswered = quizHistory.getNumAnswered();
        int result = quizHistory.getResult();


        if (this.questionNum == questionNum) { // user changed their answer on current question
            if (isCorrect) // answer is correct and wasn't before
                result++;
            else // answer is wrong and was correct before
                result--;
        }
        else if (this.questionNum < questionNum) { // user went to next question
            if (isCorrect) // answer is correct
                result++;
            else // answer is wrong
                result--;
            numAnswered++; // answered a new question
        }
        else { // user went back
            if (isCorrect)
                result++;
        ...
        this.questionNum = questionNum; // update value of question num
        updateDB(quizHistory.getId(), result, numAnswered); // update the database with new information
    }

    public long getLatestID() {
        Cursor cursor = db.query(QuizHistoryDBHelper.TABLE_QUIZZES, allColumns, null, null, null, null, "_id DESC", "1");
        return cursor.getLong(0);
    }

    public void updateDB(long id, int result, int numAnswered) {
    ...
    public QuizHistory getLatestQuizHistory() {
        ...
        cursor.moveToNext();
            if( cursor.getColumnCount() >= 10) {
                ...
                // create a new JobLead object and set its state to the retrieved values
                quiz = new QuizHistory(date, one, two, three, four, five, six, result, numAnswered);
                quiz.setId(id); // set the id (the primary key) of this object
                // add it to the list
